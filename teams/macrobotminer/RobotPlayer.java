	...
	public static void run(RobotController myRC){
		...
		while(true){
			try{
				...
				int is1337 = rc.readBroadcast(rc.getRobot().getID());
				if (is1337 == 1337) {
					isCapturer = true;
				}
...
//					}
	...
	public static void captureCode() throws GameActionException {
		...
		rc.broadcast(1337, currentChannel1337);
		if (rc.isActive()) {
	...
	public static void hqCode() throws GameActionException{
		...
		rc.broadcast(5001, closestEnc.x * 1000 + closestEnc.y); // broadcast closestEnc location
		...
		if (numAlliedSoldiers > 5) {
			if (designatedCapturer == 0) {
				...
				if (rc.readBroadcast(1337) == currentChannel1337) { // if it doesnt change
					capturerWaitCounter++;
				} else {
					currentChannel1337 = rc.readBroadcast(1337);
			...
			rc.broadcast(robotIdHash(designatedCapturer), 1337);
		}
