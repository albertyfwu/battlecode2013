	...
	public static ArrayList<MapLocation> calculatePath(RobotController rc, MapLocation start, MapLocation end) {
		...
		for (int i = 0; i < numBoxes; i++) {
			for (int j = 0; j < numBoxes; j++) {
				int i1 = bigX + i * TeamConstants.HPA_BOX_LEN;
				int j1 = bigY + j * TeamConstants.HPA_BOX_LEN;
				for (int k1 = 0; k1 < TeamConstants.HPA_BOX_LEN; k1++) {
					for (int k2 = 0; k2 < TeamConstants.HPA_BOX_LEN; k2++) {
						int i2 = i1 + k1;
						int j2 = j1 + k2;
...
//		return distanceBetweenBoxes(startBox, endBox) + mineCount * GameConstants.MINE_DEFUSE_DELAY / TeamConstants.HPA_BOX_LEN;
		return (int)Math.pow(distanceBetweenBoxes(startBox, endBox), 15) + mineCount * GameConstants.MINE_DEFUSE_DELAY / TeamConstants.HPA_BOX_LEN;
	}
	...
	public static int[] coordsToBoxIndices(int x, int y, int bigX, int bigY, int numBoxes) {
		int normX = x - bigX;
		int normY = y - bigY;
		int resultX = normX / TeamConstants.HPA_BOX_LEN;
		int resultY = normY / TeamConstants.HPA_BOX_LEN;
		// TODO: Fix this hack
		if (resultX == numBoxes) {
			resultX = numBoxes - 1;
		}
	...
	public static MapLocation boxIndicesToMapLocation(int i, int j, int bigX, int bigY) {
		int resultX = bigX + i * TeamConstants.HPA_BOX_LEN + TeamConstants.HPA_BOX_LEN / 2;
		int resultY = bigY + j * TeamConstants.HPA_BOX_LEN + TeamConstants.HPA_BOX_LEN / 2;
