			...
			try {
				if (rc.getType() == RobotType.HQ) {
					...
					if (rc.isActive()) {
						...
						if (checkNumericalAdvantage(rc, 32) ) { // if there's a numerical advantage
							//System.out.println(checkEnemyRobotsNearby(rc, 32, 1));
							if (checkEnemyRobotsNearby(rc, 32, 1) == false) { // if there aren't enemies nearby at all
								...
								Robot enemyRobot = rc.senseNearbyGameObjects(Robot.class, 14, rc.getTeam().opponent())[0];
								desiredDir = rc.getLocation().directionTo(rc.senseLocationOf(enemyRobot)); 
								Direction dir = getMovementDirection(rc, desiredDir);
								if (!badBomb(rc, rc.getLocation().add(dir))) {
									rc.move(dir);
									rc.setIndicatorString(0, "Last direction moved: "+dir.toString());
